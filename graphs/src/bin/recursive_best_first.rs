...
struct Node{
    ...
    sons: Vec<Box<Node>>,
    better_cost_father: Cell<i32>
}

impl Node {
    fn expand(&mut self, action: &str, state: &'static str) -> Node {
        ...
        let node = Node {
            ...
            sons: vec![],
            better_cost_father: {
                if let None = self.father {
                    Cell::new(9999999)
                }
                else {
                    let father = self.father.clone().unwrap();
                    let mut best_score = 9999999;
                    for brothers in father.sons {
                        if brothers.better_cost_father.get() < best_score {
                            best_score = brothers.better_cost_father.get(); 
                        }
                    }
                    if best_score > self.better_cost_father.get() {
                        father.better_cost_father
                    }
                    else {
                        Cell::new(best_score)
                    }
...
fn search_route(node: &mut Node, obj: &str) {

    if node.state == obj{
        ROUTES.lock().unwrap().push(node.route.clone());
        dbg!(ROUTES.lock().unwrap());
        dbg!(node.score);
        let nodeito = node.father.clone();
        dbg!(nodeito.unwrap().sons);
        ...
        let mut next = FRONTIER.lock().unwrap().remove(0);
        search_route(&mut next, obj)
    }
...
fn main(){
    let mut init = Node {
        ...
        sons: vec![],
        father: None,
        better_cost_father: Cell::new(999999),
    };

    search_route(&mut init, "united_states");
}
